# -*- coding: utf-8 -*-
"""
Created on Wed May 13 18:27:05 2026
...
# the sample folder.
plot = True
# Limits commonly used : limit1 = 0.005 and limit2 = 1.5
limit1 , limit2 = 0.005 , 1.5
n_sigwords = 900

### Doing the SVD thanks to the NumPy library
from _2_1extractingsample import sample_path, infos_doc, dico, doc_selected, len_selected, matrix_go
import numpy as np
import os
import matplotlib.pyplot as plt
U , s , Vh = np.linalg.svd( matrix_go , full_matrices=True )
V = np.transpose( Vh )
print('\nSVD GO done.')
...
SVD_GO_path = sample_path+"SVD_GO"
if not os.path.exists(SVD_GO_path):
    os.makedirs(SVD_GO_path)
SVD_GO_path = SVD_GO_path+'\\'
...
for i in range(4) :
    f = open(SVD_GO_path+"sigwords_GO_comp"+str(i+1)+"_withvalues.txt","w+",encoding="utf-8")
    g = open(SVD_GO_path+"sigwords_GO_comp"+str(i+1)+".txt","w+",encoding="utf-8")
...
def plot_dw1D(limit):    
    ...
    plt.savefig(SVD_GO_path+'1st component.png')
    plt.show()
    ...
    plt.savefig(SVD_GO_path+'2nd component.png')
    plt.show()
    ...
    plt.savefig(SVD_GO_path+'3rd component.png')
    plt.show()
    ...
    plt.savefig(SVD_GO_path+'4th component.png')
    plt.show()
...
def plot_dw2D():
    ...
    plt.savefig(SVD_GO_path+'1st and 2nd components.png')
    plt.colorbar()
    plt.show()
    ...
    plt.savefig(SVD_GO_path+'1st and 3rd components.png')
    plt.colorbar()
    plt.show()
    ...
    plt.savefig(SVD_GO_path+'2nd and 3rd components.png')
    plt.colorbar()
    plt.show()
...
def plot_dw2Dp():
    ...
    plt.savefig(SVD_GO_path+'1st and 4th components.png')
    plt.colorbar()
    plt.show()
    ...
    plt.savefig(SVD_GO_path+'2nd and 4th components.png')
    plt.colorbar()
    plt.show()
    ...
    plt.savefig(SVD_GO_path+'3rd and 4th components.png')
    plt.colorbar()
    plt.show()
...
def plot_ww(limit) :
    ...
    for i in range(4):
        ...
        plt.xlabel("Year")
        plt.ylabel("Getis Ord's z-scores")
        plt.savefig(SVD_GO_path+'Ten most significatives words of component '+str(i+1)+'.png')
        plt.show()
...
def plot_lengths() :
    ...
    plt.savefig(SVD_GO_path+'Abs values, component 1.png')
    plt.show()
    ...
    plt.xlabel('Length of the document')
    plt.ylabel('Aboslute value on the component')
    plt.savefig(SVD_GO_path+'Abs values, component 2.png')
    plt.show()
    ...
    plt.xlabel('Length of the document')
    plt.ylabel('Aboslute value on the component')
    plt.savefig(SVD_GO_path+'Abs values, component 3.png')
    plt.show()
    ...
    plt.xlabel('Length of the document')
    plt.ylabel('Aboslute value on the component')
    plt.savefig(SVD_GO_path+'Abs values, component 4.png')
    plt.show()
